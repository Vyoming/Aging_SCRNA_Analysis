...
Prop_table<- prop.table(x = table(UPAR_obj$Cell_Type, UPAR_obj$Treatment), margin = 2)
...
DE_all <- FindMarkers(UPAR_obj, ident.1 = "UPAR+", ident.2 = "UPAR-", test.use = "MAST", logfc.threshold = .01, min.pct = .01, assay = 'SCT')
...
write.csv(DE_all, paste0('All_UPAR_scrna_DE.csv')) 
DE_all<- DE_all[!grepl("Xist", rownames(DE_all)), ]
if (max(-log(DE_all$p_val_adj)) < 321){
  ylim_num <- max(-log(DE_all$p_val_adj))} else {
    ylim_num <- 320}
EnhancedVolcano(DE_all, lab = rownames(DE_all), x = 'avg_log2FC', y = 'p_val_adj', title = 'UPAR+ vs UPAR-',
...
celltypes <- c(levels(as.factor(UPAR_obj$Cell_Type)))
Idents(UPAR_obj) <- UPAR_obj$Cell_Type
for(i in celltypes){
  subset_cell <- subset(UPAR_obj,  idents = i)
  Idents(subset_cell) <- subset_cell$Treatment
  DE_subset <- FindMarkers(subset_cell, ident.1 = "UPAR+", ident.2 = "UPAR-",  test.use = "MAST", logfc.threshold = .1, min.pct = .01, assay = 'SCT')
  ...
  write.csv(DE_subset, paste0(i,'_UPAR_ATAC_DE.csv')) 
  ...
  if (max(-log(DE_subset$p_val_adj)) < 321){
    ylim_num <- max(-log(DE_subset$p_val_adj))} else {
      ylim_num <- 320}
  EnhancedVolcano(DE_subset, lab = rownames(DE_subset), x = 'avg_log2FC', y = 'p_val_adj', title = 'UPAR vs UPAR-',
...
#Minimize clustering for some plotting functionality:
Idents(UPAR_obj) <- UPAR_obj$Cell_Type
...
sen_BRD4<- c(
  "ANKRD30A", "EPHA7", "LINC01225", "KBTBD8", "IL37", "LINC00395", "LINC01419", "CD180", "TRIM33", 
  "ITGBL1", "ADO", "PABPC1P2", "KCNN2", "FBXO28", "TNS3", "LINC00477", "GPR68", "LINC01080", "MMP10", 
  "LOC101929497", "MSR1", "MMP3", "LOH12CR1", "ZHX2", "PHF3", "DUSP6", "SNTG1", "EYA3", "CHST2", 
  "LOC101927616", "LINC01449", "LINC01449", "TMEM106B", "LOC101927822", "LINC01222", "HAO1", "ZNF385B", 
  "LOC100507406", "LINC00656", "DUSP6", "PRDM1", "CLDN12", "LOC102467213", "CITED4", "LINC01393", 
  "TEX41", "LOC728084", "IL1B", "GRB10", "NRG1-IT3", "LOC100289673", "EDN1", "ERAP2", "MMP20", 
  "SLC9A9-AS1", "SLC24A1", "CCAT1", "VAC14", "LINC01080", "CXCL8", "SERPINB7", "ADAM28", "ETS1", 
  "FAM155A-IT1", "LINC01449", "XCR1", "POU2F2", "RAB27B", "STC1", "OTUD4", "C9orf92", "SLC6A4", 
  "PTEN", "E2F7", "NMBR", "TMX3", "CNIH3", "QPCT", "SPP2", "MME", "MAPKAP1", "SOX11", "AMN1", 
  "LOC728084", "FGF20", "VEGFC", "PALMD", "LOC101928569", "VEGFC", "SERPINB2", "SPCS3", "ETAA1", 
  "CCDC130", "TXLNB", "FEZF2", "CDKN2B", "ARHGAP5-AS1", "ADAMTS12", "MIR4776-1", "RAB31", "RND3", 
  "CORO2B", "AREG", "SPCS3", "NAMA", "INHBA", "BET1", "GABRA2", "MIR31HG", "C2CD4A", "DCBLD2", 
  "GSAP", "LOC101926928", "MIR4500", "LOC101929532", "FAM155A-IT1", "MRPL15", "CASC20", "ANGPT2", 
  "LINC00603", "MPHOSPH6", "UBE2E1", "MIR137HG", "RALA", "AFF3", "CLDN1", "C12orf74", "ZC3H12C", 
  "FMN2", "THAP1", "LOC645434", "LOC100130476", "EHF", "LOC728084", "TGFBR2", "FRMD6", "TMC1", 
  "CBLB", "ARSG", "TRPS1", "LSAMP-AS1", "LINC00900", "KCNJ15", "MARCKS", "MIR6881", "KLHDC9", 
  "SNORD114-20", "ABCA1", "SLC8A1-AS1", "HS3ST3A1", "TNFSF8", "CWC27", "GREM1", "LOC101927378", 
  "LOC646736", "SYNE1-AS1", "CSF2", "TBX3", "LINC01048", "EGFL6", "NTM", "PCAT1", "NPC1", "IRAK2", 
  "SFR1", "TNFAIP3", "DUSP10", "NKX2-6", "MYO16-AS1", "TGFB2-OT1", "EREG", "LINC00443", "SCOC", 
  "LINC01452", "NCEH1", "NGF", "SUMO1P1", "TNIP1", "PCAT1", "PRNP", "GMDS", "ANTXR2", "BCAT1", 
  "LINC01108", "MAST4", "ZNF438", "HGF", "FAM19A3", "HS3ST3A1", "ARHGAP24", "LOC101927865", 
  "LOC100507065", "GLIS3-AS1", "SERP2", "SH3PXD2A-AS1", "DSEL", "ANKDD1B", "SLC25A37"
...
acute_Inflammatory <- intersect(All_Genes, acute_inflam)
...
sen_BRD4 <- intersect(All_Genes, sen_BRD4)

mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[acute_Inflammatory, ], na.rm = TRUE), dist = 'norm')
if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
  ...
  UPAR_obj@meta.data$acute_Inflammatory <- mean.exp
}


mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[Inflammatory_Response, ], na.rm = TRUE), dist = 'norm')
if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
  cat("Cell names order match in 'mean.exp' and 'object@meta.data':\n", 
      "adding gene set mean expression values in 'object@meta.data$gene.set.score'")
  UPAR_obj@meta.data$Inflammatory_Response <- mean.exp
}


mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[senescence, ], na.rm = TRUE), dist = 'norm')
if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
  cat("Cell names order match in 'mean.exp' and 'object@meta.data':\n", 
      "adding gene set mean expression values in 'object@meta.data$gene.set.score'")
  UPAR_obj@meta.data$senescence <- mean.exp
}

mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[stem_score, ], na.rm = TRUE), dist = 'norm')
if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
  cat("Cell names order match in 'mean.exp' and 'object@meta.data':\n", 
      "adding gene set mean expression values in 'object@meta.data$gene.set.score'")
  UPAR_obj@meta.data$stem_score <- mean.exp
}

mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[MHC_score, ], na.rm = TRUE), dist = 'norm')
if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
  cat("Cell names order match in 'mean.exp' and 'object@meta.data':\n", 
      "adding gene set mean expression values in 'object@meta.data$gene.set.score'")
  UPAR_obj@meta.data$MHC_score <- mean.exp
}

mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[Hormone, ], na.rm = TRUE), dist = 'norm')
if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
  cat("Cell names order match in 'mean.exp' and 'object@meta.data':\n", 
      "adding gene set mean expression values in 'object@meta.data$gene.set.score'")
  UPAR_obj@meta.data$Hormone <- mean.exp
}

mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[Absorption, ], na.rm = TRUE), dist = 'norm')
if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
  cat("Cell names order match in 'mean.exp' and 'object@meta.data':\n", 
      "adding gene set mean expression values in 'object@meta.data$gene.set.score'")
  UPAR_obj@meta.data$Absorption <- mean.exp
}

mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[antimicrobial, ], na.rm = TRUE), dist = 'norm')
if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
  cat("Cell names order match in 'mean.exp' and 'object@meta.data':\n", 
      "adding gene set mean expression values in 'object@meta.data$gene.set.score'")
  UPAR_obj@meta.data$antimicrobial <- mean.exp
}

mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[genes_stem_prolif, ], na.rm = TRUE), dist = 'norm')
if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
  cat("Cell names order match in 'mean.exp' and 'object@meta.data':\n", 
      "adding gene set mean expression values in 'object@meta.data$gene.set.score'")
  UPAR_obj@meta.data$genes_stem_prolif <- mean.exp
}

mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[genes_antimicrobial, ], na.rm = TRUE), dist = 'norm')
if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
  cat("Cell names order match in 'mean.exp' and 'object@meta.data':\n", 
      "adding gene set mean expression values in 'object@meta.data$gene.set.score'")
  UPAR_obj@meta.data$genes_antimicrobial <- mean.exp
}

mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[genes_hormone_secretion, ], na.rm = TRUE), dist = 'norm')
if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
  cat("Cell names order match in 'mean.exp' and 'object@meta.data':\n", 
      "adding gene set mean expression values in 'object@meta.data$gene.set.score'")
  UPAR_obj@meta.data$genes_hormone_secretion <- mean.exp
}

mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[genes_nutrient_absorption, ], na.rm = TRUE), dist = 'norm')
if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
  cat("Cell names order match in 'mean.exp' and 'object@meta.data':\n", 
      "adding gene set mean expression values in 'object@meta.data$gene.set.score'")
  UPAR_obj@meta.data$genes_nutrient_absorption <- mean.exp
}

mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[Inflammatory_score_new, ], na.rm = TRUE), dist = 'norm')
if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
  cat("Cell names order match in 'mean.exp' and 'object@meta.data':\n", 
      "adding gene set mean expression values in 'object@meta.data$gene.set.score'")
  UPAR_obj@meta.data$Inflammatory_score_new <- mean.exp
}
mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[sen_mayo, ], na.rm = TRUE), dist = 'norm')
if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
  cat("Cell names order match in 'mean.exp' and 'object@meta.data':\n", 
      "adding gene set mean expression values in 'object@meta.data$gene.set.score'")
  UPAR_obj@meta.data$sen_mayo <- mean.exp
}
mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[sen_BRD4, ], na.rm = TRUE), dist = 'norm')
if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
  cat("Cell names order match in 'mean.exp' and 'object@meta.data':\n", 
      "adding gene set mean expression values in 'object@meta.data$gene.set.score'")
  UPAR_obj@meta.data$sen_BRD4 <- mean.exp
}
...
UPAR_obj$acute_Inflammatory
i = 'acute_Inflammatory'
for(i in score_lsit) {
  selected_cells <- names(UPAR_obj$Cell_Type_condense)
  # selected_cells <- rownames(UPAR_obj@meta.data[UPAR_obj$Treatment == c('Control','Arasco'),] )
  
  vln_data <- FetchData(UPAR_obj,
                        vars = c(i,"Treatment", "Cell_Type_condense"),
                        cells = selected_cells,
                        slot = "data")
  vln_data <- melt(vln_data)
  vln_data <- vln_data[c('Treatment', 'Cell_Type_condense', 'value')]
  
  All <- VlnPlot(UPAR_obj,group.by = 'Cell_Type_condense' , split.by = "Treatment", features = i, pt.size = 0, assay = "MAGIC_RNA", cols = c('#7CA1CC' ,'#FF4902'), log = FALSE, split.plot = TRUE) + 
    theme(legend.position = 'none')  +
    theme(axis.line = element_line(size = .3),axis.title.x = element_blank(), axis.ticks = element_line(size = .3), text = element_text(size=7), axis.text.x = element_text(size = 6), axis.text.y = element_text(size = 6)) #+ scale_y_continuous(expand = expansion(mult = c(0, .1)))
  dodge <- position_dodge(width = .9)
  All <- All + geom_boxplot(width=0.3,outlier.shape = NA, coef = 0, lwd=.2, position = dodge) +
    stat_compare_means(data= vln_data, aes(x = Cell_Type_condense, y = value, fill = Treatment), hide.ns = TRUE, method = "t.test",  label = "p.signif", size = 1)  
  All$layers[[1]]$aes_params$size = .15
  ggsave(file = paste0('UPAR_scrna_vln_CondenseCellType_', i, '.pdf'), plot=All, width=3, height=2.5, units="in")
}
...
Sen_score_list$SASP
  ...
  for (i in score_list){
    ...
    mean.exp <- zscore(colMeans(x = UPAR_obj@assays$MAGIC_RNA@data[sen_score, ], na.rm = TRUE), dist = 'norm')
    if (all(names(x = mean.exp) == rownames(x = UPAR_obj@meta.data))) {
    cat("Cell names order match in 'mean.exp' and 'object@meta.data':\n", 
        "adding gene set mean expression values in 'object@meta.data$gene.set.score'")
    UPAR_obj@meta.data$sen_score <- mean.exp
    }
     ...
     All <- VlnPlot(UPAR_obj,group.by = 'Cell_Type' , split.by = "Treatment", features = 'sen_score', pt.size = 0, assay = "MAGIC_RNA", cols = c('#7CA1CC' ,'#FF4902'), log = FALSE, split.plot = TRUE) + 
        theme(legend.position = 'none')  +
        theme(axis.line = element_line(size = .3),axis.title.x = element_blank(), axis.ticks = element_line(size = .3), text = element_text(size=7), axis.text.x = element_text(size = 6), axis.text.y = element_text(size = 6)) #+ scale_y_continuous(expand = expansion(mult = c(0, .1)))
      dodge <- position_dodge(width = .9)
      All <- All + geom_boxplot(width=0.3,outlier.shape = NA, coef = 0, lwd=.2, position = dodge) +
...
#gene set enrichment:
library(fgsea)
celltypes <- unique(UPAR_obj$Cell_Type)
i = 'Macrophage'
for(i in celltypes){
DE_treat <- read.csv( paste0(i,'_UPAR_ATAC_DE.csv'))
DE_treat <- DE_treat[DE_treat$p_val_adj < .05 & (DE_treat$avg_log2FC > .25 | DE_treat$avg_log2FC < -.25), ]
DE_treat$gene_id <- DE_treat$X %>% convert_mouse_to_human_symbols()
res2 <- DE_treat %>% 
...
write.csv(gsea_output,paste0('Upar_sort_GSEA_',i,'.csv'))
}
colnames(gsea_output)

pathways <- c('GOBP_LIPID_LOCALIZATION', 'GOBP_ATP_METABOLIC_PROCESS', 'GOBP_CELLULAR_RESPIRATION','GOBP_OXIDATIVE_PHOSPHORYLATION', 'GOCC_RESPIRASOME', 'GOBP_MRNA_METABOLIC_PROCESS', 'GOBP_CELL_DIVISION', 'GOBP_MRNA_PROCESSING', 'GOMF_CADHERIN_BINDING', 'GOMF_CELL_ADHESION_MOLECULE_BINDING')
...
All <- ggplot(data = gsea_output1,aes(x=pathway,y=NES)) +  theme_vyom +
  ...
  coord_flip() + 
  scale_y_continuous(limits = c(-3.2,3.2),expand = expansion(mult = c(0, 0)), breaks = scales::breaks_extended(n = 3)) +
  labs(y= "Normalized Enrichment Score", x="Pathway") 
All <- All + geom_hline(yintercept = 0)
All
ggsave(file = paste0('UPAR_enteroctye_GSEA.pdf'), plot=All, width=5, height=3, units="in")

meta.check <- UPAR_obj@meta.data
...
sen_cells <-WhichCells(object = UPAR_obj, expression = sen_mayo1 > .01)
DimPlot(UPAR_obj, label=T,group.by = 'Treatment', cells.highlight= list(sen_cells),  cols.highlight = c("darkblue"),cols= "grey")
...
ggplot(Prop_Table, aes(fill=Var1, y=Freq, x=Var2)) + 
  geom_bar(position="fill", stat="identity")+theme_vyom +scale_fill_manual(values = c("#7CA1CC", "#FF4902"))
ggsave( "UPARsort_senescent_cell_proportion.pdf", width=2.5, height=3, units="in")
